...
- 状态沉淀：最终特质、最终标签。
- 隐藏阴影叙事状态终值 `shadow_*_final`；字段语义见 [SHADOW_STATE.md](SHADOW_STATE.md)。
...
- 该年的事件效果摘要和自然流变摘要。
- 七项阴影叙事状态的 `shadow_*_before` / `shadow_*_after`，用于检查跨年连续性。
...
- 精确到单条事件的 `shadow_*_before` / `shadow_*_after`；同一年触发多条事件时也分别记录。
...
- 9 个代表性出生世代：1840、1870、1900、1925、1950、1970、1990、2010、2020；最后一个世代专门用于覆盖 21 世纪后半叶。
- 男女两种性别。
- 乡村与城市两种开局环境。
...
`balanced` 会完整组合出生世代、性别、城乡、家庭资源与代表地区，并让三种属性档位在组合间均匀轮换，共 1188 局。其他模式：

- `--matrix smoke`：108 局快速检查，每个核心出生/性别/城乡/阶级单元轮换一个地区和属性档位。
- `--matrix full`：把属性档位也加入全笛卡尔积，共 3564 局。
...
分层 years/events 表额外记录事件触发前后的所在地、婚姻、子女、教育、职业状态，以及教育轨道、当前/已完成学段、学习模式、职业段数、主要身份和最后一次人生状态转移，供跨年连续性审查使用。叙事列同时记录当年层级、领域、连续纹理年、距上次结构事件的年份和活跃主线，能直接定位一生在哪个阶段退化成了连续日常切片。

审查器还单列 2036—2132 年的虚构未来史密度、未来事件定义覆盖率，按七个时间窗以及出生世代、城乡、家庭资源、地区分群寻找空白。未来样本达到 200 个人年后，严格门禁要求未来史事件率不低于 8%，未来史定义覆盖率不低于 45%。
...
- 1990 年后出生且活到 30 岁的人物是否真正进入过职业状态，防止未来内容只有岗位文案却没有职业履历。
...
- 转折、后果、历史压力与日常纹理占比；成年后结构事件密度，以及连续六年以上只有纹理的完整人生样本。
- 终年 P10/P50/P90、婴幼儿/未成年/40 岁前结束率、死亡年龄尖峰和年龄上限截尾。
- 主动伤害在十五年内是否留下责任、关系、记忆或自我辩解的痕迹、只靠结尾悔悟收束、阴影状态跳变与同一人生矛盾。未受惩罚本身不是错误，现实允许逃脱。
- 阴影结构结局中道歉/归还/补偿、明确惩罚、未结清、硬化或逃脱各占多少；默认拒绝让大多数灰暗人生被一次修复或处罚整理成道德结论。
- 阴影事件是否在单个人生中过密，以及性别、阶层、城乡触发率是否显著失衡；身份可以改变处境，不能直接充当道德原因。
- 确定性调度的结构链是否在人物仍存活时越过续段窗口；关系、职业或地点变化不能让已写下的后果凭空消失。
- 低龄成人视角、无子女人物的亲属称谓、退休后当前在职语义，以及高置信时代词是否同逐年状态冲突。

引擎为少量明确标记的儿童、中年与晚年结构链提供叙事安全网：当连续纹理已经过长时，允许这些链绕过一次原本的生命周期抽样概率，再由既有年龄、时代、地点、阶层和状态条件筛选。安全网不适用于死亡、犯罪阴影或普通稀有事件，目的是修复“候选池已经为空”，不是让所有人经历同一故事。

审查结束后应生成一组可直接通读的完整人生，而不是只翻 CSV 单元格：
...
抽样器会自动纳入各出生世代、性别、城乡、家庭资源、地区与属性档位的中位寿命样本，以及最早死亡、最长寿、阴影事件最多、伤害/愧疚/硬化/自我欺骗/信任债最高、平常年最多、日常纹理最多和审查器在不同世代发现的首批问题局，并按年份输出玩家实际看到的完整正文。

启发式风险进入人工审查队列；CSV 关联错误、明示条件冲突、明显性别生理冲突与高置信时代词穿越属于阻断项。用 `--strict` 开启自动门禁：

严格门禁还要求阴影状态没有跳变/断裂、身份分群没有显著道德偏置、过密人生不超过 5%，且样本足够时至少 88% 的主动伤害在十五年内留下可观察痕迹。它不要求受罚、变善或悔悟。
...
默认数值门槛为：重复可见文案不超过 3%、头部十事件不超过 45%、平常年不超过 35%、日常纹理不超过 72%、结构事件不低于 25%、连续六年以上只有纹理的人生不超过 20%、年龄上限截尾不超过 2%、事件定义覆盖不低于 45%，且每个样本足够的现代出生世代中，活到 30 岁仍从未进入职业状态的人物不超过 35%。阴影链中修复性结局不超过 55%，被修复或惩罚明确结清的结局不超过 75%；这两项只防止单向道德化，不要求把人物写得更恶。人生渗透率超过 55% 的非生命周期事件和由单一文案支撑的高频事件都必须清零。可分别用 `--max-repeat-copy-rate`、`--max-top10-share`、`--max-quiet-rate`、`--max-texture-rate`、`--min-structural-rate`、`--max-modern-career-void-rate`、`--max-shadow-reparative-closure-rate`、`--max-shadow-settled-closure-rate`、`--max-long-texture-streak-life-rate`、`--max-overpenetrating-events`、`--max-copy-monoculture-events`、`--max-age-cap-rate`、`--min-event-coverage` 覆盖。阈值是回归护栏，不替代对完整人生的人工通读。
