...
审查器还单列 2036—2120 年的虚构未来史密度、未来事件定义覆盖率，按六个时间窗以及出生世代、城乡、家庭资源、地区分群寻找空白。未来样本达到 200 个人年后，严格门禁要求未来史事件率不低于 8%，未来史定义覆盖率不低于 45%。
...
抽样器会自动纳入各出生世代、性别、城乡、家庭资源、地区与属性档位的中位寿命样本，以及最早死亡、最长寿、阴影事件最多、平常年最多、日常纹理最多和审查器在不同世代发现的首批问题局，并按年份输出玩家实际看到的完整正文。
...
默认数值门槛为：重复可见文案不超过 3%、头部十事件不超过 45%、平常年不超过 35%、日常纹理不超过 72%、结构事件不低于 25%、连续六年以上只有纹理的人生不超过 20%、年龄上限截尾不超过 2%、事件定义覆盖不低于 45%。此外，人生渗透率超过 55% 的非生命周期事件和由单一文案支撑的高频事件都必须清零。可分别用 `--max-repeat-copy-rate`、`--max-top10-share`、`--max-quiet-rate`、`--max-texture-rate`、`--min-structural-rate`、`--max-long-texture-streak-life-rate`、`--max-overpenetrating-events`、`--max-copy-monoculture-events`、`--max-age-cap-rate`、`--min-event-coverage` 覆盖。阈值是回归护栏，不替代对完整人生的人工通读。
