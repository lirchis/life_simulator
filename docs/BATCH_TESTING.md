...
默认数值门槛为：重复可见文案不超过 3%、头部十事件不超过 45%、平常年不超过 35%、日常纹理不超过 72%、结构事件不低于 25%、年龄上限截尾不超过 2%、事件定义覆盖不低于 45%。可分别用 `--max-repeat-copy-rate`、`--max-top10-share`、`--max-quiet-rate`、`--max-texture-rate`、`--min-structural-rate`、`--max-age-cap-rate`、`--min-event-coverage` 覆盖。阈值是回归护栏，不替代对完整人生的人工通读。
