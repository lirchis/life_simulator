...
出生年份从 1900 开始后，省份、城市层级、家庭阶层都不能只用当代口径。设计上采用两层字段：
...
- 城市层级保留稳定 code，如 `village/county/tier1`，但 label 随时代变化：1900 年显示“府城 / 商埠”，1950 年显示“地级市”，2000 年显示“普通城市”。
- 家庭阶层按时代维护不同 label 和 code：1900 年可出现“无地雇农 / 佃户 / 富农 / 买办 / 士绅”，1949 后可出现“贫下中农 / 国营厂矿职工 / 知识分子 / 海外关系 / 地主富农成分”，1978-1991 可出现“乡镇企业 / 个体户 / 干部子弟 / 先富家庭”，1992 后再加入“外出务工 / 下岗职工”等转轨语境，2000 后可出现“留守农村 / 进城务工 / 体制内 / 专业人士 / 新经济家庭”等。
...
4. 从候选事件中加权随机抽取。
5. 每年根据年龄阶段抽取 1 到 3 个事件。

`priority` 用来保证关键节点事件出现，比如出生、入学、高考、毕业、第一份工作、退休、重大疾病、死亡等。普通事件不应该滥用 `priority`。

如果同一年存在多个高 `priority` 关键事件冲突，只取最高优先级事件；同优先级再根据最终权重加权随机抽取一个。被选中的关键事件先占用事件槽位，剩余槽位再从普通候选池中抽取。
...
如果本年存在必须出现的高 `priority` 关键事件，应按上述冲突规则选出一个并先占用事件槽位；剩余槽位再从普通候选池加权随机抽取。
...
1. 出生年份范围先定为 `1900` 到 `2020`。
2. 省份第一版全量支持，并保留 `other` 兜底。
3. `environment` 每年动态计算。
4. 每年事件数量根据年龄阶段决定，范围为 1 到 3 条。
