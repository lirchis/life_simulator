...
玩家手选或随机生成初始状态
  ...
  -> 计算环境和年度自然变化
  -> 筛选候选事件
  -> 按转折 / 后果 / 历史压力 / 日常纹理计算叙事层级
  -> 结合当前主线、连续纹理年与距上次结构事件的年份调整候选池
  -> 计算事件权重
  -> 加权随机抽取事件
  -> 自动抽取事件结果
...
4. 使用叙事状态调节候选池与权重，避免多年连续抽中只提供日常纹理的事件。
5. 从候选事件中加权随机抽取。
6. 每年只抽取 1 个主叙事事件。
...
事件的 `narrativeTier` 分为：

- `turning_point`：改变教育、职业、关系、居所、健康边界或人生方向的转折。
- `historical_pressure`：战争、政策、灾害、公共制度与时代环境对个人生活施加的结构性压力。
- `consequence`：由既有事件或状态引出的延续、代价、修复与收束。
- `texture`：物件、饭桌、通勤、邻里与身体小波动等日常纹理。
- `chronicle`：隐藏固定年谱的逐年作者文本，不进入普通人生的比例门禁。

旧事件未显式声明时，引擎会根据生命周期状态改变、事件依赖、效果强度、历史窗口和事件类别推断层级；新写的跨年内容应显式声明。`narrativeDomain` 把事件归入教育、职业、家庭、关系、生计、迁居、健康、历史或自我等领域。

`PlayerState.narrative` 记录最近一次层级、距上次结构事件的年份、连续纹理年、各层计数和仍在发挥作用的领域主线。转折与历史压力会打开一条主线，后果事件会获得同领域加权，收束事件可用 `narrativeThread.close` 关闭它。成年人生若连续多年只有纹理，而当年存在合条件的结构事件，选择器会优先结构事件；它不会凭空制造不满足年代、年龄、地点或人物状态的事件。

叙事分层不是要求每年发生灾难。日常仍占据一生的大量年份，但它应成为节奏与质地，而不是替代人物处境的推进。
